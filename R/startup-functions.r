...
# 2014/07/17: Compute empiric CDF of a numeric variable.
...
# of a score variable. But it turned out that this only works when the population from which the sample should be drawn, should be uniformly distributed
...
# - cdf
{
	x.tab = as.data.frame(table(x))
	colnames(x.tab) = c("x", "Freq")
	# Remove the factor property from puntaje
	x.tab$x = as.numeric( as.character(x.tab$x) )
	# Add the start and end values of the distribution of puntaje
	if (x.tab$x[1] != 0) { 
		x.tab = rbind(c(0,0), x.tab)
	}
	if (x.tab$x[nrow(x.tab)] != 1) {
		x.tab = rbind(x.tab, c(1,0))
	}
	#table(x.tab)
	x.tab$pdf = x.tab$Freq / sum(x.tab$Freq)
	x.tab$cdf = cumsum(x.tab$Freq) / sum(x.tab$Freq)
	#print(x.tab)	# OK
...
#						Added for this the option mode="name" in the exists() function that checks for existence of xlab and ylab.
{
  ...
  if (is.null(paramsList$type)) paramsList$type = "l"
